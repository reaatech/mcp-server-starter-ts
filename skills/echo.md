...
- **Tool call:**
  ...
  - **Expected response:**
    ...
    ```
    Note: The sanitization middleware strips dangerous patterns like `<script>` tags from the input before it reaches the tool.
...
- Logs are automatically redacted by the pino logger, but avoid sending PII in tool arguments
